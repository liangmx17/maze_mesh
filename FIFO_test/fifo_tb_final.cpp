...
int main(int argc, char** argv) {
    ...
    std::cout << "Key insight: Read data appears immediately when rd_en is asserted!" << std::endl;
    ...
    // Read back 5 items with CORRECT timing
    std::cout << "\nReading 5 items (correct timing):" << std::endl;

    // Start read and capture data immediately in same cycle
    dut->rd_en = 1;
    tick();  // First read data (0x10) appears immediately!
    ...
    for (int i = 1; i < 5; i++) {
        ...
        std::cout << "Read " << i << ": 0x" << std::hex << (int)read_val
                  << " -> Count=" << (int)dut->count << std::endl;
    ...
    // Read the last item that was on rd_data
    uint8_t last_val = dut->rd_data;
    actual_data.push_back(last_val);
    std::cout << "Read 5: 0x" << std::hex << (int)last_val << std::endl;
    ...
    for (int i = 0; i < 3; i++) {
        ...
        uint8_t read_val = dut->rd_data;  // Read from previous cycle
        actual_data.push_back(read_val);
    ...
    // Read the last item from rd_data
    uint8_t final_val = dut->rd_data;
    ...
    std::cout << "• Read: 1-cycle latency (data appears immediately after rd_en assertion)" << std::endl;
