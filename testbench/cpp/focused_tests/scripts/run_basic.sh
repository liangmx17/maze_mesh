#!/bin/bash
# ====================================================================
# MAZE Network Node - Basic Functionality Test Runner
# ====================================================================
#
# Script to run basic functionality tests with configurable options
#
# Usage: ./run_basic.sh [options]
#   Options:
...
print_usage() {
    echo "MAZE Network Node - Basic Functionality Test Runner"
    echo "================================================="
    echo ""
...
build_test() {
    log_info "Building basic functionality test..."
    cd "$TEST_DIR"
    ...
    if ! make test_basic; then
        log_error "Failed to build basic functionality test"
        exit 1
    fi

    log_success "Basic functionality test built successfully"
}
...
run_test() {
    ...
    if test_output=$(make run_basic HP="$HP" VP="$VP" VCD_FLAG="$VCD" VERBOSE_FLAG="$VERBOSE" 2>&1); then
        local test_end=$(date +%s)
        local test_duration=$((test_end - test_start))

        log_success "Basic functionality test completed successfully in ${test_duration}s"
        log_info "Test results saved to: basic_test_results.txt"

        if [ -n "$VCD" ]; then
            log_info "VCD waveform saved to: test_basic.vcd"
        fi
        ...
        if [ -f "basic_test_results.txt" ]; then
            ...
            echo "====================="
            tail -n 10 basic_test_results.txt
        fi
        ...
        log_error "Basic functionality test failed after ${test_duration}s"
...
main() {
    ...
    if [ ! -f "$TEST_DIR/build/bin/test_basic" ]; then
