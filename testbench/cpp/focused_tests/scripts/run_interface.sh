#!/bin/bash
# ====================================================================
# MAZE Network Node - Interface Test Runner
# ====================================================================
#
# Script to run interface protocol tests with configurable options
#
# Usage: ./run_interface.sh [options]
#   Options:
...
print_usage() {
    echo "MAZE Network Node - Interface Test Runner"
    echo "======================================="
    echo ""
...
build_test() {
    log_info "Building interface test..."
    cd "$TEST_DIR"
    ...
    if ! make test_interface; then
        log_error "Failed to build interface test"
        exit 1
    fi

    log_success "Interface test built successfully"
}
...
run_test() {
    log_info "Running interface protocol test..."
    ...
    if test_output=$(make run_interface HP="$HP" VP="$VP" VCD_FLAG="$VCD" VERBOSE_FLAG="$VERBOSE" 2>&1); then
        local test_end=$(date +%s)
        local test_duration=$((test_end - test_start))

        log_success "Interface test completed successfully in ${test_duration}s"
        log_info "Test results saved to: interface_test_results.txt"

        if [ -n "$VCD" ]; then
            log_info "VCD waveform saved to: test_interface.vcd"
        fi

        # Display key results
        if [ -f "interface_test_results.txt" ]; then
            echo ""
            log_info "Test Results Summary:"
            echo "====================="
            tail -n 15 interface_test_results.txt
        fi
        ...
        log_error "Interface test failed after ${test_duration}s"
...
main() {
    ...
    if [ ! -f "$TEST_DIR/build/bin/test_interface" ]; then
    ...
    # Run the test
    if run_test; then
        log_success "Interface test completed successfully!"
        exit 0
    else
        log_error "Interface test failed!"
        exit 1
    fi
}
