#!/bin/bash
# ====================================================================
# MAZE Network Node - Fault Tolerance Test Runner
# ====================================================================
#
# Script to run fault tolerance tests with configurable options
#
# Usage: ./run_fault.sh [options]
#   Options:
...
print_usage() {
    echo "MAZE Network Node - Fault Tolerance Test Runner"
    echo "=============================================="
    echo ""
...
build_test() {
    log_info "Building fault tolerance test..."
    cd "$TEST_DIR"
    ...
    if ! make test_fault; then
        log_error "Failed to build fault tolerance test"
        exit 1
    fi

    log_success "Fault tolerance test built successfully"
}
...
run_test() {
    log_info "Running fault tolerance test..."
    ...
    if test_output=$(make run_fault HP="$HP" VP="$VP" VCD_FLAG="$VCD" VERBOSE_FLAG="$VERBOSE" 2>&1); then
        local test_end=$(date +%s)
        local test_duration=$((test_end - test_start))

        log_success "Fault tolerance test completed successfully in ${test_duration}s"
        log_info "Test results saved to: fault_test_results.txt"

        if [ -n "$VCD" ]; then
            log_info "VCD waveform saved to: test_fault.vcd"
        fi

        # Display key results
        if [ -f "fault_test_results.txt" ]; then
            echo ""
            log_info "Test Results Summary:"
            echo "====================="
            tail -n 15 fault_test_results.txt
        fi
        ...
        log_error "Fault tolerance test failed after ${test_duration}s"
...
main() {
    ...
    if [ ! -f "$TEST_DIR/build/bin/test_fault" ]; then
    ...
    # Run the test
    if run_test; then
        log_success "Fault tolerance test completed successfully!"
        exit 0
    else
