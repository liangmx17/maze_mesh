#!/bin/bash
# ====================================================================
# MAZE Network Node - Pipeline Timing Test Runner
# ====================================================================
#
# Script to run pipeline timing tests with configurable options
#
...
print_usage() {
    echo "MAZE Network Node - Pipeline Timing Test Runner"
    echo "==============================================="
    echo ""
...
build_test() {
    log_info "Building pipeline timing test..."
    cd "$TEST_DIR"
    ...
    if ! make test_pipeline; then
        log_error "Failed to build pipeline timing test"
        exit 1
    fi

    log_success "Pipeline timing test built successfully"
}
...
run_test() {
    log_info "Running pipeline timing test..."
    ...
    if test_output=$(make run_pipeline HP="$HP" VP="$VP" VCD_FLAG="$VCD" VERBOSE_FLAG="$VERBOSE" 2>&1); then
        local test_end=$(date +%s)
        local test_duration=$((test_end - test_start))

        log_success "Pipeline timing test completed successfully in ${test_duration}s"
        log_info "Test results saved to: pipeline_test_results.txt"

        if [ -n "$VCD" ]; then
            log_info "VCD waveform saved to: test_pipeline.vcd"
        fi

        # Display key results
        if [ -f "pipeline_test_results.txt" ]; then
            echo ""
            log_info "Test Results Summary:"
            echo "====================="
            tail -n 15 pipeline_test_results.txt
        fi
        ...
        log_error "Pipeline timing test failed after ${test_duration}s"
...
main() {
    ...
    if [ ! -f "$TEST_DIR/build/bin/test_pipeline" ]; then
    ...
    # Run the test
    if run_test; then
        log_success "Pipeline timing test completed successfully!"
        exit 0
    else
        log_error "Pipeline timing test failed!"
        exit 1
    fi
}
