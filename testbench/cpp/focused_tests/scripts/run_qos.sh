#!/bin/bash
# ====================================================================
# MAZE Network Node - QoS Arbitration Test Runner
# ====================================================================
#
# Script to run QoS arbitration tests with configurable options
#
# Usage: ./run_qos.sh [options]
#   Options:
...
print_usage() {
    echo "MAZE Network Node - QoS Arbitration Test Runner"
    echo "=============================================="
    echo ""
...
build_test() {
    log_info "Building QoS arbitration test..."
    cd "$TEST_DIR"
    ...
    if ! make test_qos; then
        log_error "Failed to build QoS arbitration test"
        exit 1
    fi

    log_success "QoS arbitration test built successfully"
}
...
run_test() {
    log_info "Running QoS arbitration test..."
    ...
    if test_output=$(make run_qos HP="$HP" VP="$VP" VCD_FLAG="$VCD" VERBOSE_FLAG="$VERBOSE" 2>&1); then
        local test_end=$(date +%s)
        local test_duration=$((test_end - test_start))

        log_success "QoS arbitration test completed successfully in ${test_duration}s"
        log_info "Test results saved to: qos_test_results.txt"

        if [ -n "$VCD" ]; then
            log_info "VCD waveform saved to: test_qos.vcd"
        fi

        # Display key results
        if [ -f "qos_test_results.txt" ]; then
            echo ""
            log_info "Test Results Summary:"
            echo "====================="
            tail -n 15 qos_test_results.txt
        fi
        ...
        log_error "QoS arbitration test failed after ${test_duration}s"
...
main() {
    ...
    if [ ! -f "$TEST_DIR/build/bin/test_qos" ]; then
    ...
    # Run the test
    if run_test; then
        log_success "QoS arbitration test completed successfully!"
        exit 0
    else
        log_error "QoS arbitration test failed!"
        exit 1
    fi
}
